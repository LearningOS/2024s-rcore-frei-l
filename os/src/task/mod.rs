...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.task_start_time = Some(get_time_us() / 1000);
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].task_start_time.is_none(){
                inner.tasks[next].task_start_time = Some(get_time_us() / 1_000);
            }
    ...
    /// get a copy of tcb from current task
    fn get_current_tcb(&self) -> TaskControlBlock{
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
